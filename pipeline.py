...
def run_pipeline(pdf_directory: str = 'data/pdfs', 
                ...
                embeddings_directory: str = 'data/embeddings') -> bool:
    """
    Run the complete financial analysis pipeline.
    
    Note: The parser engine selection is now handled internally by PDFParser.
    """
    try:
        logger.info("=" * 60)
        ...
        # Initialize the new PDFParser without the engine parameter
        parser = PDFParser()
        financial_data = []
        notes_data = []
...
def main():
    ...
    # The 'parser-engine' argument is now removed since the new parser is a single multi-tool engine
